...
describe('insertionSort', () => {
    ...
    it('should return a sorted copy of an array of very big size', () => {
        const ARRAY_SIZE = 1000;
        const intArr = Array.from({ length: ARRAY_SIZE }, (e, idx) => idx % 100);

        testSort(intArr, intCmpAsc);
        testSort(intArr, intCmpDesc);

        const strArr = Array.from({ length: ARRAY_SIZE }, (e, idx) => String.fromCharCode(idx % 255));

        testSort(strArr, strcmpAsc);
        testSort(strArr, strcmpDesc);
    });
